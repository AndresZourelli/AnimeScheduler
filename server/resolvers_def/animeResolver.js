...
const resolverAnime = {
  Query: {
    ...
    getLicensors: async (_, args) => {
      try {
        const { rows } = await pool.query("select * from licensors");
        return rows;
      } catch (error) {
        console.log(error);
        return [];
  ...
  Mutation: {
        ...
        if (raw_data.names) {
          raw_data.names.forEach((other_name) => {
            const {
              ...
              "insert into alt_names (fk_anime_id, alt_name) values ($1, $2) returning *",
              [result.anime_id, other_name.alt_name]
            );
          });
        }
    ...
    editAnime: async (_, args) => {
      ...
      try {
        ...
        const {
          rows: original_data,
        } = await client.query("select * from animes where anime_id=$1", [
          anime_id,
        ]);
        if (!original_data[0]) {
          return {
        ...
        if (raw_data.licensors) {
          const licensors_delete = await Promise.all(
            raw_data.licensors
              .filter((licensor) => !licensor.checked)
              .map(async (obj) => {
                const {
        ...
        if (raw_data.producers) {
          const producers_delete = await Promise.all(
            raw_data.producers
              .filter((producer) => !producer.checked)
              .map(async (obj) => {
                const {
        ...
        if (raw_data.names) {
          const {
            ...
            "select * from alt_names where fk_anime_id=$1",
            [anime_id]
          );
            ...
            raw_data.names
              ...
              .map(async (obj) => {
                const {
                  ...
                  "delete from alt_names where alt_name=$1 and fk_anime_id=$2",
                  [obj.alt_name, anime_id]
                );
              })
          );
    ...
    deleteAnime: async (_, args) => {
      try {
        const {
          rows,
        } = await pool.query("delete from animes where anime_id=$1", [
          args.anime_id,
        ]);
        if (!rows[0]) {
          return {
